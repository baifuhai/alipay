...
public class WXPay {
    ...
    public Map<String, String> fillRequestData(Map<String, String> reqData) throws Exception {
        ...
        if (SignType.MD5 == config.getSignType()) {
            reqData.put("sign_type", WXPayConstants.MD5);
        } else if (SignType.HMACSHA256 == config.getSignType()) {
    ...
    /**
     * 处理HTTPS API返回数据，转换成Map对象。return_code为SUCCESS时，验证签名。
     *
    ...
    public Map<String, String> processResponseXml(String xmlStr) throws Exception {
        ...
        if (respData.containsKey("return_code")) {
            returnCode = respData.get("return_code");
        } else {
    ...
    public Map<String, String> unifiedOrder(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        ...
        if(config.notifyUrl() != null) {
            reqData.put("notify_url", config.notifyUrl());
        }
        String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
        return this.processResponseXml(respXml);
    }
